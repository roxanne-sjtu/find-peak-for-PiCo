...
void selectElementsFromIndices(std::vector<int> in, std::vector<int> indices, std::vector<int>& out)
{
	...
	for(int i=0; i<indices.size(); ++i){
		...
		if (in.size() < indices[i]) continue;
		if (in[indices[i]]) out.push_back(in[indices[i]]);
		// std::cout<<"Value:" << " " << in[indices[i]] << std::endl;
	}
...
void PeakFinder::findPeaks(std::vector<float> x0, std::vector<int>& peakInds, bool includeEndpoints, float extrema)
{
	...
	else
	{
		...
		if(x.size()>2)
		{
			minMagIdx = distance(x.begin(), std::min_element(x.begin(), x.end()));	//find the min 	extreme point
			minMag = x[minMagIdx];				
	...
	if(len>2)
	{
    	...
    	while(ii < len)
    	{
            ...
            {            	
				...
				// decide the right and left bound of peaks
                // int leftBoundary = tempLoc;
				// std::cout << "temploc"<< tempLoc << std::endl;
                // while (leftBoundary > 0 && std::abs(x[leftBoundary]) >= 0.1)
                // {
                //     leftBoundary--;
                // }
                // std::cout << "leftboundary"<< leftBoundary << std::endl;
                // int rightBoundary = tempLoc;
                // while (rightBoundary < len - 1 && std::abs(x[rightBoundary]) >= 0.1)
                // {
                //     rightBoundary++;
                // }
				// std::cout << "rightboundary"<< rightBoundary << std::endl;
                // peakRanges.push_back({leftBoundary, rightBoundary});
    	...
    	{        	
			...
			// std::cout << "peakLocTmp values: ";
			// for (size_t i = 0; i < peakLocTmp.size(); ++i) {
			// 	std::cout << peakLocTmp[i] << " ";
			// }
			// std::cout << std::endl;
